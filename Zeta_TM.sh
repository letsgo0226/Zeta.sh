...
def execute(path,data,limit,q,n,m,trace=False):
 ...
 z=dict(MODE='trace' if trace else 'run',STATE=state,HEAD=head,STEPS=i,HALTED=state in halts,REASON=reason,TAPE=out,TAPE_ORIGIN=origin,FINAL_CONFIGURATION=final,TM_CLASS='finite deterministic single-tape',CONFIG_ENCODING_EXACT=True,ZETA_TRACE_EMBEDDING=True,X_PROJECTION_INJECTIVE=False,**base())
 if trace:z['TRACE']=T
 return z
def point(n,m,q):
...
def omega():return dict(MODE='omega',DISCRETE_TM_DYNAMICS=True,ZETA_ANALYTIC_EMBEDDING=True,CONFIG_ENCODING_EXACT=True,X_PROJECTION_INJECTIVE=False,EXACT_ZETA_VALUE_ATTAINED_BY_FINITE_FLOAT_EXECUTION=False,LIMIT_EXISTS='not_assumed',ATTAINED_BY_FINITE_EXECUTION=False,**base())
def addnum(x):
...
for k in ('self','zero','omega'):sp.add_parser(k)
