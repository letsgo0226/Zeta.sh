#!/usr/bin/env python3
"""Reference-target X/Y→ζ recursive raster→SVG vectorizer.
Exact bytes, ζ analytic proposal features, and measured visual alignment stay distinct.
Pillow required; mpmath enables ζ phase. Native SVG only: no embedded raster.
"""
import argparse,hashlib,json,math,colorsys
...
def L(a,b):return sum(abs(x[0]-y[0])+abs(x[1]-y[1])+abs(x[2]-y[2]) for x,y in zip(a,b))/(765*len(a))
...
def prop(G,k,th,t,n):
 ...
 cx=n*u;cy=n*v;r=n*(.012+.12/(1+.004*k));a=th+.08*ph;x=cx+r*math.cos(a);y=cy-r*math.sin(a);x2=cx-r*math.cos(a);y2=cy+r*math.sin(a);c=sample(t,cx*t.width/n,cy*t.height/n);c=c if ink(c) else (43,220,255);w=1+(k%2)
 return [('L',(x,y,x2,y2),c,w),('R',(cx-r,cy-r,cx+r,cy+r),c,w),('E',(cx-r,cy-r,cx+r,cy+r),c,w)],a
def svg(P,w,h,n):
 O=[]
 for k,a,c,sw in P:
  sx=w/n;sy=h/n;col='#%02x%02x%02x'%c
...
def main():
 a=argparse.ArgumentParser();a.add_argument('target');a.add_argument('-n','--steps',type=int,default=2048);a.add_argument('--grid',type=int,default=128);a.add_argument('-o','--output',default='Reference_AutoAligned.svg');q=a.parse_args();raw=Path(q.target).read_bytes();G=E(raw);assert D(G)==raw;t=Image.open(q.target).convert('RGB');N=q.grid;tf=F(t,N);th=((G%1000003)/1000003-.5)*math.tau;P=[];cur=L(tf,F(render(P,N),N));accepted=0
 for k in range(q.steps):
  C,th=prop(G,k,th,t,N);best=None
  for p in C:
   Q=P+[p];e=L(tf,F(render(Q,N),N));best=min(best or (e,Q), (e,Q),key=lambda x:x[0])
  if best[0]<cur:P,cur=best;accepted+=1
 Path(q.output).write_text(svg(P,t.width,t.height,N),encoding='utf-8');print(json.dumps({'target_sha256':hashlib.sha256(raw).hexdigest(),'target_size':[t.width,t.height],'exact_roundtrip':D(G)==raw,'accepted_primitives':accepted,'steps':q.steps,'grid':N,'measured_rgb_l1':cur,'zeta_backend':bool(mp),'primitive_set':['line','rect','ellipse'],'color_sampled_from_target':True,'aspect_ratio_preserved':True,'pure_vector':True,'embedded_raster':False,'pixel_identity_claimed':False,'rh_proved':False,'omega_attained':False,'open':True,'final':False},separators=(',',':')))
